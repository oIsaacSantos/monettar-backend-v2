...
function hasPassedBRT(date, endTime) {
    const nowUTC = Date.now();
    const [year, month, day] = date.split("-").map(Number);
    const [h, m] = endTime
        ? endTime.slice(0, 5).split(":").map(Number)
...
async function getDashboardSummary(businessId) {
    ...
    for (const a of appointments ?? []) {
        ...
        const inMonth = a.appointment_date?.startsWith(month);
        ...
        if (inMonth) {
            if (futureConfirmed)
                futureConfirmedRevenue += revenue;
            if (futurePending)
                futurePendingRevenue += revenue;
        }
    }
