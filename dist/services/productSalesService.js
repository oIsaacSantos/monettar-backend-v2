...
exports.ProductSaleError = void 0;
...
exports.ProductSaleError = ProductSaleError;
...
async function assertClientBelongsToBusiness(businessId, clientId) {
    if (!clientId)
        return null;
    ...
    if (!data)
        throw new ProductSaleError("Cliente nao encontrado para este negocio.");
    return clientId;
}
async function assertAppointmentBelongsToBusiness(businessId, appointmentId) {
    if (!appointmentId)
        return null;
    const { data, error } = await supabase
        .from("appointments")
    ...
    if (error)
        throw new ProductSaleError(error.message, 500);
    if (!data)
        throw new ProductSaleError("Atendimento nao encontrado para este negocio.");
    return appointmentId;
}
async function getActiveProductForSale(businessId, productId) {
    const { data, error } = await supabase
        .from("products")
    ...
    if (error)
        throw new ProductSaleError(error.message, 500);
    if (!data)
        throw new ProductSaleError("Produto nao encontrado.");
    if (data.active === false)
        throw new ProductSaleError("Produto inativo nao pode ser vendido.");
    return {
        ...data,
...
async function decrementStockWithRetry(businessId, productId, quantity) {
...
async function restoreStock(productId, businessId, quantity) {
    ...
    if (productError || !product) {
        throw new ProductSaleError(productError?.message ?? "Nao foi possivel reverter o estoque.", 500);
    }
...
async function getProductSales(businessId, month) {
...
async function createProductSale(businessId, payload) {
    if (!payload.productId)
        throw new ProductSaleError("productId obrigatorio.");
    ...
    const appointmentId = await assertAppointmentBelongsToBusiness(businessId, payload.appointmentId);
    const { product } = await decrementStockWithRetry(businessId, payload.productId, quantity);
    const unitPrice = payload.unitPrice === undefined || payload.unitPrice === null || payload.unitPrice === ""
        ? product.sale_price
