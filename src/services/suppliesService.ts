...
function validateSupplyInput(payload: {
  name?: string;
  unit?: unknown;
  totalCostPaid?: unknown;
  packageQuantity?: unknown;
}) {
  const name = payload.name?.trim();
  if (!name) throw new Error("Nome do insumo e obrigatorio.");
  if (!payload.unit) throw new Error("Unidade do insumo e obrigatoria.");
...
export async function createSupply(
  businessId: string,
  payload: { name: string; unit: unknown; totalCostPaid: unknown; packageQuantity: unknown }
) {
